...
class AgilityViewController: UIViewController {
    ...
    @IBOutlet weak var agilityTableView: UITableView!
    ...
    override func viewDidLoad() {
        ...
        self.agilityTableView.separatorColor = UIColor.clear
        
        NotificationCenter.default.addObserver(forName:NSNotification.Name(rawValue: Constants.NotificationKey.AGILITY_NOTIFICATION_KEY), object:nil, queue:nil, using:loadedAgilityHeroes)
        
        registerCell()
        
        HeroModel.shared().loadHeroes(heroType: Constants.FirebaseNode.AGILITY)
        
    }
    
    func registerCell(){
        let cellNib = UINib(nibName : "HeroTableViewCell", bundle : nil)
        agilityTableView.register(cellNib, forCellReuseIdentifier: Constants.HeroCellId.HERO_CELLID)
    }
    
    func loadedAgilityHeroes(notification: Notification) -> Void{
        ...
        if let data = notification.userInfo![Constants.EventKey.DATA] as? [Heros] {
            ...
            self.agilityTableView.reloadData()
...
extension AgilityViewController : UITableViewDataSource {
...
extension AgilityViewController : UITableViewDelegate {
